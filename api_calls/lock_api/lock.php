...
$check_login_user = mysqli_query($connect_db, "SELECT * FROM `tbl_users` WHERE `user_loginid`='$get_users_loginid' LIMIT 1") or die(mysqli_error($connect_db));
    if(mysqli_num_rows($check_login_user) > 0){
        ...
        if($get_details['user_status'] === '1'){
            ...
            $users_role = $get_details['user_type'];    // get & set users role or type
            ...
            $users_full_name = $users_fname.' '.$users_lname;   // get & set users fullname
            $db_harshed_password = $get_details['user_passcode'];   // get users passcode
            $users_login_name = $get_details['user_loginid'];   // get & set username
