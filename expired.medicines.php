...
<div class="content-page">
                                ...
                                <tr>
                                    ...
                                    <td>
                                     ...
                                     <td>
                                        ...
                                        <!-- <button type="submit" class="btn btn-info btn-sm font-weight-bold refurbish" id="refurbish" data-id="<?php echo $get_product_id; ?>">Refurbish</button> -->
                                    </td>
                                    </td>
    ...
    <div class="card-box d-none">
                            <table class="table table-sm table-hover m-0 table-centered nowrap w-100" id="expired-list">
                                <thead>
                                    <tr style="color: #145388;" >
                                        <th class="font-weight-bold">No</th>
                                        <th class="font-weight-bold">Medicine Name</th>
                                ...
                                <?php
                                        $counter = 1;
                                        // $getProductInfo = mysqli_query($connect_db,"SELECT * FROM `tbl_medicines` ORDER BY mid ASC")or die(mysqli_error($connect_db));
                                        $getProductInfo = mysqli_query($connect_db,"SELECT * FROM `tbl_medicines` WHERE medicine_expiry_date <= curdate() ORDER BY medicine_expiry_date ASC")or die(mysqli_error($connect_db));
                                        while($product_info = mysqli_fetch_array($getProductInfo)){  
                                        $get_product_id = $product_info['mid'];
                                        $get_product_name = $product_info['medicine_name'];
    ...
    function disposeMedicine(supplierId) {
       swal.fire({
			...
			text: "Permanently dispose off this medicine from the system!... Are you Sure?",
			type: 'error',
			...
			preConfirm: function() {
			  return new Promise(function(resolve) {
			     ...
			     .fail(function(){
			     	swal.fire('Oops...', 'Sorry.. Something went wrong with ajax!', 'error');
			     });
			  });
		    },
    ...
    // refurbish specific medicine
    function refurbishMedicine(supplierId) {
       swal.fire({
			title: 'Refurbish Medicine?',
			text: "This action will refurbish medicine and set status to 'Available', making it accessible and active again!.. Are you Sure?",
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#145388',
			cancelButtonColor: '#d33',
			confirmButtonText: 'Yes, Refurbish!',
			showLoaderOnConfirm: true,
			  
			preConfirm: function() {
			  return new Promise(function(resolve) {
			     $.ajax({
			   		url: 'api_calls/medicine_api/refurbish.medicine.php',
			    	type: 'POST',
			       	data: 'delete='+supplierId,
			       	dataType: 'json'
			     })
    ...
    <script>
        $('#print').click(function(){
            ...
            var content = $('#expired-list').clone()
            ns.append(content)
