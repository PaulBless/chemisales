<?php 
    ...
    $february_sales_array = [];
    $february_profit_array = [];

    ## get february
    $february_subtotal = 0;
    $february_profit = 0;
    $medicine_total = 0;
    $fetch_february = mysqli_query($connect_db,"SELECT * FROM tbl_special_sales WHERE MONTH(sales_datetime) = '02' AND YEAR(sales_datetime) = YEAR(CURDATE()) ORDER BY (sales_datetime)") or die(mysqli_error($connect_db));
    ...
    while($data = mysqli_fetch_array($fetch_february)){
        ...
        while($february_sales = mysqli_fetch_array($get_sales))
        {
            $medicine_id = $february_sales['medicineId'];
            $medicine_qty = $february_sales['medicineQty'];
            $medicine_total += $february_sales['profit'];
        ...
        // $february_profit= $medicine_total;
    }
    array_push($february_profit_array, $february_profit);
    array_push($february_sales_array, $february_subtotal);
    endif;
